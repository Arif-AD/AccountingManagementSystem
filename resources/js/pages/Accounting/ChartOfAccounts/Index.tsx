            ...
            <div className="mx-auto max-w-7xl">
                <div className="flex flex-col justify-between gap-5 border-b border-[#dce4d9] pb-7 sm:flex-row sm:items-end">
                    <div>
                        ...
                        <h1 className="mt-2 text-3xl font-semibold tracking-tight">Chart of Accounts</h1>
                        <p className="mt-2 text-sm text-[#617268]">
                            The account structure that will organize every journal entry.
                        </p>
                    </div>

                    {canEdit && (
                        <button type="button" className="w-fit rounded-lg bg-[#17352d] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#285348]">
                            + Add account
                        </button>
                    )}
                </div>
                    ...
                    <table className="w-full min-w-[700px] text-left text-sm">
                        <thead className="border-b border-[#dce4d9] text-xs uppercase tracking-[0.12em] text-[#789083]">
                            <tr>
                                ...
                                <th className="px-5 py-4 font-semibold">Parent account</th>
                        ...
                        <tbody className="divide-y divide-[#e5ebe4]">
                            {accounts.map((account: ChartOfAccount) => (
                                <tr key={account.id} className="transition hover:bg-[#f0f5ed]">
                                    ...
                                    <td className="px-5 py-4 text-[#617268]">{account.parent?.name ?? 'Top level account'}</td>
                                    <td className="px-5 py-4">
                ...
                <p className="mt-4 text-xs text-[#789083]">
                    {accounts.length} seeded accounts · Ready for future journal workflows
                    {!canEdit && ' · (View only)'}
                </p>
            </div>
        </AuthenticatedLayout>
